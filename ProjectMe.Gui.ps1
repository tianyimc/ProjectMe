try {
  ...
  (Get-Control 'ArticlesNavButton').Add_Click({ Set-Page articles }); (Get-Control 'MaintenanceNavButton').Add_Click({ Set-Page maintenance }); (Get-Control 'TimelineNavButton').Add_Click({ Set-Page timeline }); (Get-Control 'AboutNavButton').Add_Click({ Show-Message "$($script:info.description)`n`n版本：$(Get-DisplayVersion $script:info)`n作者：$($script:info.author)`n$($script:info.copyright)" '关于 ProjectMe' })
  ...
  $script:window.Add_Closed({ Stop-Preview }); Refresh-Articles; Set-Page articles; [void]$script:window.ShowDialog()
} catch {
